$(document).ready(function(){
	...
	if(typeof($(".fixnavi")[0]) != 'undefined'){
		...
		//posArr == [561, 1077, 1593, 1853, 2369, 2885] 
	...
	//购票详情页
	$("#ctrLeft, #ctrRight").hover(function(){
		$(this).addClass("hover");
	},function(){
		$(this).removeClass("hover");
	});
	$("#slimClose").click(function(){
		$(".js-slims").hide();
	});

	//{Start:购票}
	var selCity;
	$("#js_ch_city").bind('click',function(){
		$(".tk-selCity").show();
		selCity = false;
		return false;
	});
	...
	$(document).click(function(){
		if(!selCity) $(".tk-selCity").hide();
	});
		//幻灯片切换
		if(typeof($(".ticket-slide")[0]) != 'undefined'){
			$(".ticket-num").switchable('.ticket-slide > ul > li', { 
				effect: 'fade',
				circular:'true', 
				speed: .5
			}).autoplay();
			var api = $(".ticket-num").switchable();
			$('.tk-right').click(function(){
				api.next();
			});
			$('.tk-left').click(function(){
				api.prev();
			});
		}
	if($(".notict li").size() > 1){
		setInterval('newsListScroll()', 5000);
	}
    setInterval('AutoScroll("#js_autoScroll ul", "70")', 3700);

    //正在热映
    $('#tickScr-trigger-one').switchable('#tickScr-content-one > ul > li', {
		triggerType: 'click',
		effect: 'scroll',
		steps: 6,
		visible: 6
	}).autoplay().carousel();
	var tickScr_trigger_one = $("#tickScr-trigger-one").switchable();
	$('#tickScr-arrow-one-right').click(function(){
		tickScr_trigger_one.next();
	});
	$('#tickScr-arrow-one-left').click(function(){
		tickScr_trigger_one.prev();
	});
    //即将上映
    $('#tickScr-trigger-two').switchable('#tickScr-content-two > ul > li', {
		triggerType: 'click',
		effect: 'scroll',
		steps: 6,
		visible: 6
	}).autoplay().carousel();
	var tickScr_trigger_two = $("#tickScr-trigger-two").switchable();
	$('#tickScr-arrow-two-right').click(function(){
		tickScr_trigger_two.next();
	});
	$('#tickScr-arrow-two-left').click(function(){
		tickScr_trigger_two.prev();
	});
    //预告片\图片
    $('#tickScr-trigger-three').switchable('#tickScr-content-three > ul > li', {
		triggerType: 'click',
		effect: 'scroll',
		steps: 5,
		visible: 5
	}).carousel();
	var tickScr_trigger_three = $("#tickScr-trigger-three").switchable();
	$('#tickScr-arrow-three-right').click(function(){
		tickScr_trigger_three.next();
	});
	$('#tickScr-arrow-three-left').click(function(){
		tickScr_trigger_three.prev();
	});
	...
	$(".filmList li a").bind({
		mouseover:function(){
			var seLeft = Number($(this).offset().left + $(this).width() + 15),
				seTop  = Number($(this).offset().top),
				rnd    = new Date().getTime();
			delayLoad = setTimeout(function(){
				loadLayoutTick(seLeft, seTop, function(){
					$.get("http://www.viswow.com/api/price.php",{'rnd':rnd},function(data){
						data = data['data'];
						var ul = '<ul>';
						for(var i = 0;i < data.length;i++){
							ul += '<li><img src="'+data[i]['icon']+'" /><a href="'+data[i]['url']+'">'+data[i]['name']+'在线选座<span>'+data[i]['price']+'元</span>起</a></li>'
						}
						ul += '</ul>';
						$("#ajaxload").remove();
						$("#exceptick").append(ul);
		...
		mouseout:function(){
			removePick = setTimeout(function(){
				$("#exceptick").remove();
			}, 500);
			clearTimeout(delayLoad);
		}
	});
	function loadLayoutTick(left, top, callback){
		var tkLayout = '<div id="exceptick" style="left:'+left+'px;top:'+top+'px;">'
							+'<span id="ajaxload"></span>'
							+'<em></em>'
						+'</div>';
		$("body").append(tkLayout);
		if(callback){
			callback();
		}
	};
...
function searchform(){
	var q = $('input[name=q]').val(), t = $('input[name=t]').val();
	if(t!=''){
