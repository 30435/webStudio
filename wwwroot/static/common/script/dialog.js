...
; (function(window, undefined) {
	...
	$.fn.animate = function(prop, speed, easing, callback) {
		...
		var $this = this,
		overflow, fx, parts, start, end, unit, opt = {
			speed: speed,
			easing: easing,
			callback: function() {
	...
	$.fx.tick = function() {
		var timers = $.timers;
		for (var i = 0; i < timers.length; i++) { ! timers[i]() && timers.splice(i--, 1)
	...
	$.noop = $.noop ||
	function() {};
	var _box, _thisScript, _skin, _path, _count = 0,
	_$window = $(window),
	_$document = $(document),
	_$html = $('html'),
	_$body = $(function() {
	...
	artDialog.fn = artDialog.prototype = {
		...
		_init: function(config) {
			var that = this,
			DOM, icon = config.icon,
			iconBg = icon && (_isIE6 ? {
				png: '../css/icons/' + icon + '.png'
			}: {
				backgroundImage: 'url(\'' + config.path + '/../css/icons/' + icon + '.png\')'
			});
			that._isRun = true;
			that.config = config;
		...
		content: function(msg, url) {
			...
			if (isIframe) {
				...
				if (document.addEventListener) iframe.addEventListener('load', $ready, false);
				else if (document.attachEvent) iframe.attachEvent('onload', $ready);
		...
		size: function(width, height) {
			var maxWidth, maxHeight, scaleWidth, scaleHeight, that = this,
			config = that.config,
			DOM = that.DOM,
			wrap = DOM.wrap,
			main = DOM.main,
		...
		function() {
			var i = 0,
			elem, png, pngPath, runtimeStyle, path = artDialog.defaults.path + '/skins/',
...
} ((window.jQuery && (window.art = jQuery)) || window.art, this));; (function($) {
	var _dragEvent, _use, _$window = $(window),
	_$document = $(document),
	_elem = document.documentElement,
	...
	_use = function(event) {
		var limit, startWidth, startHeight, startLeft, startTop, isResize, api = artDialog.focus,
		config = api.config,
		DOM = api.DOM,
		wrap = DOM.wrap,
		title = DOM.title,
		...
		_dragEvent.onstart = function(x, y) {
			...
			_$document.bind('dblclick', _dragEvent.end); ! _isIE6 && _isLosecapture ? title.bind('losecapture', _dragEvent.end) : _$window.bind('blur', _dragEvent.end);
		...
		_dragEvent.onend = function(x, y) {
			_$document.unbind('dblclick', _dragEvent.end); ! _isIE6 && _isLosecapture ? title.unbind('losecapture', _dragEvent.end) : _$window.unbind('blur', _dragEvent.end);
		...
		limit = (function() {
			var maxX, maxY, wrap = api.DOM.wrap[0],
