...
(
	...
	public function notify()
	{
		...
		$this->currentPayment = $this->paymentInfos[$paymentCode];
		$this->currentPayment['returnUrl'] = $this->appInfos['pay']['url'] . 'frontpay/respond/' . $paymentCode;
		$this->currentPayment['notifyUrl'] = $this->appInfos['pay']['url'] . 'frontpay/notify/' . $paymentCode;
	...
	public function change($payType = '', $infos = array())
	{
		if (empty($this->loginedUserInfo)) {
			$this->_messageInfo('您还没有登陆，请先登录平台', $this->appInfos['passport']['url'] . 'uwebgame/login');
		}
		$payType = empty($payType) ? $this->input->post('payType') : $payType;
		$payFields = array(
			'topaymonth' => array('username', 'money', 'paymonthId'),
			'towebgame' => array('username', 'money', 'webgameCode', 'serverId'),
		);
		...
		foreach ($payFields[$payType] as $payField) {
			...
			if ($payField != 'serverId' && empty($payInfos[$payField])) {
