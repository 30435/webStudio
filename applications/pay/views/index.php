...
$(document).ready(function(){
	...
	$("#username").formValidator({onShow:"",onFocus:"用户名至少5个字符,最多10个字符",onCorrect:"该用户名有效"}).inputValidator({min:5,max:10,onError:"你输入的用户名非法,请确认"})//.regexValidator({regExp:"username",dataType:"enum",onError:"用户名格式不正确"})
	    .ajaxValidator({
		dataType : "jsonp",
		async : false,
		jsonp:"callback",
			...
			<form method="post" autocomplete="off" name="form1" id="form1" action="<?php echo $this->baseUrl; ?>frontpay/change/">
				...
				<ul class="form_list">
					...
					<li class="form_list_li">
						...
						<div class="txt_tips"><span id="serverIdTip"></span></div>
					</li>
					...
					<?php if ($this->payType == 'topaymonth') { $i = 1; echo '<input type="hidden" name="money" id="money" value="' . $this->paymonthInfo['money'] . '" />'; if (count($this->webgamePaymonthInfos) > 1) { ?>
					<li class="form_list_li">
					...
					<li class="form_list_li">
						<a class="ui_btn_large_em" href="javascript: $('#form1').submit(); void(0);" id="payButton"><span>确定充值</span></a>
					</li>
				</ul>
			</form>
			</div>
